...
class Handlers:
    ...
    async def booking_ask(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.message.text == 'Time(s)':
            ...
            booked, booked_time = context.user_data['booker'].attempt_booking()

            if booked:
                await context.bot.send_message(chat_id=update.effective_chat.id, text=f"Successfully booked at {booked_time}:00 on {context.user_data['booker'].get_selected_day()}/{datetime.now().month}!")
